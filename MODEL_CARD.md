...
- **Checkpoint evaluated in this card**: `models/best_thoravis.pt`, saved at epoch 7/20 of the run described in the "Results" section of [README.md](README.md) (not committed to git — regenerate via `python -m src.train --epochs 20 --batch_size 64 --lr 2e-5`, ~4.7h on an RTX 5060).
...
- **Split**: 73,546 train / 12,978 val / 25,596 test. Val and test are NIH's own train_val/test partition (not re-derived); val was additionally split out of the train_val pool by this project (see README's Health Check for the one known gap here — the split is by row index, not by patient ID, so a patient whose images straddle the val boundary could appear in both train and val; NIH's own dataset wrapper doesn't expose patient ID, so this hasn't been fixed).
...
|---|---|---|
| Validation (12,978 images) | 0.787 | Used to pick epoch 7 as "best" — carries a small optimistic bias by construction |
| **Test (25,596 images, never touched)** | **0.753** | The honest number |

**Calibration** (test set): pooled Expected Calibration Error 0.0121 — but read the README's Calibration section before trusting that alone. The pooled number is dominated by the large mass of easy, correctly-low-confidence negatives across 15 labels; the reliability diagram shows real, systematic overconfidence in the 0.4–0.9 predicted-probability range specifically — the range that would actually drive a decision. A fitted temperature of ≈1.07 suggests only mild *average* miscalibration, which understates the problem in that actionable range.
...
1. **Close the patient-level split gap.** Requires a data-layer change (NIH ChestX-ray14's classification config drops patient ID); scoped as its own task in the README roadmap, not yet done.
...
3. **Recalibrate for the actionable range**, not just fit a single global temperature — the reliability diagram shows the miscalibration is concentrated at 0.4–0.9, not uniform.
4. **Add early stopping.** This checkpoint was hand-picked at epoch 7 after visibly overfitting past it; a real training pipeline shouldn't depend on a human reading a loss curve after the fact.
