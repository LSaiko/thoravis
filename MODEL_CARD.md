...
3. ~~**Recalibrate for the actionable range.**~~ Done — `fit_isotonic_calibration(..., per_class=True)` fits a separate isotonic curve per pathology instead of one pooled curve. The pooled version made things worse (ECE 0.0275 vs. temperature scaling's 0.0112 overall; 0.2019 vs. 0.1660 in the 0.4–0.9 band) by averaging 15 pathologies' different miscalibration shapes into one curve. Per-class removes exactly that averaging: test-set ECE 0.0118 overall, **0.0627 in the 0.4–0.9 band** — a 2.6x improvement over temperature scaling and the best of every method tried. **Per-class isotonic regression is now the recommended calibration method** for this checkpoint's actionable confidence range; see `results/calibration_comparison.txt` and the README's Calibration section.
